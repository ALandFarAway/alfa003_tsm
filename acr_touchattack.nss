...
{   object oTarget = GetSpellTargetObject();
	SendMessageToAllDMs("Touch Attack firing");
	SendMessageToPC(oTarget, "Touch Attack firing");
	...
	{SendMessageToAllDMs("Ghost firing");
	 SendMessageToPC(oTarget, "Ghost firing");
		...
		if(GetObjectType(oTarget) == OBJECT_TYPE_CREATURE)
		{
			...
			if(nTouch)
			{
				...
				SendMessageToAllDMs("Ghost healed 5hp");
				SendMessageToPC(oTarget, "Ghost healed 5hp");
	...
	{SendMessageToAllDMs("Allip firing");
	 SendMessageToPC(oTarget, "Allip firing");
		...
		if(GetObjectType(oTarget) == OBJECT_TYPE_CREATURE)
		{
			...
			if(nTouch)
			{
				...
				ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectTemporaryHitpoints(5)), OBJECT_SELF);
				SendMessageToAllDMs("Allip healing");
	 			SendMessageToPC(oTarget, "Allip healing");
